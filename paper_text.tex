...
\begin{equation}\label{eq:density}
	\rho^* = n_g - M + \frac{ \bar \rho_0}{\beta W(0)}.
\end{equation}
...
\begin{equation}\label{eq:M_nT}
	\bar M = \frac{\rho_{n}}{\beta W(0)} - (\rho^* - n_g),
\end{equation}
where
\begin{align} \label{eq:ro_nT}
	& \rho_{n} = - 2 \left(\frac{g_3^2 - 2g_2 g_4}{g_4^2} \right)^{\frac{1}{2}} \cos \left( \frac{\alpha_n}{3} + \frac{\pi}{3} \right), \\
	& \alpha_n = \arccos \left( \left( - \frac{9 g_4^4}{\left( 2 g_2 g_4 - g_3^2\right)^3}\right)^{\frac{1}{2}} (n_g - \rho^*)\right). \nonumber 
\end{align}
...
\begin{equation}\label{eq:eosNT}
	Pv\beta = E_\eta (\rho^*,T) + \bar M \rho_{n} + \frac{\tilde D(0)}{2} \rho_{n}^2 - \frac{a_4}{24} \rho_{n}^4.
\end{equation}
The quantity $E_\nu (\rho^*, T)$ in \eqref{eq:ro_nT} is the function $E_\mu (M,T)$ \eqref{eq:E_mu} rewritten in terms of density and temperature taking into account the expression \eqref{eq:M_nT}
\begin{eqnarray}\label{eq:E_nu}
	E_\nu (\rho^*, T) & = & - \frac{\ln (2\pi \beta W(0))}{2 N_v}  +  g_0 
	\nonumber\\
	&& - \frac{\beta W(0)}{2} 
...
\begin{eqnarray}
	...
	P^* & = & (1 + \tau)T^*_c \bigg[ E_\eta (\rho^*,T) 
	\nonumber\\
	&& + \bar{M} \rho_{n} + \frac{\tilde D(0)}{2} \rho_{n}^2 - \frac{a_4}{24} \rho_{n}^4\bigg].
\end{eqnarray}

Figure~\ref{fig1} shows the isotherms for the pressure $P^*$ versus density $\rho^*$ (left-hand side) and versus the effective chemical potential $M$ (right-hand side).
\begin{figure}[h!]
	\centering 
	\includegraphics[width=0.446\textwidth]{f1a1.pdf} 
	\includegraphics[width=0.45\textwidth]{f1b1.pdf} 
	\vskip-3mm\caption{Isotherms of the reduced pressure $P^*$ as a function of the density $\rho^*$ (on the left-hand side) and a function of the effective chemical potential $M$ (on the right-hand side) at $T \geq T_c$ represented by black lines. Thick grey lines on both figures correspond to isotherms of pressure at $T < T_c$ based on the results taken from~\cite{KozlovskiiDobush2020}. 
	}\label{fig1}
\end{figure}
...
\end{equation}
The quantity $K^*_T$ is dimensionless and is of order unity, except at the critical point itself, where it is divergent. It is expressed in terms of the reduced quantities $P^*$ and $\rho^*$ as follows
\begin{eqnarray}
	K^*_T & = & \frac{\varepsilon}{{\rho^*}^2} \left(\frac{\partial \rho^*}{\partial \mu}\right)_T
	\nonumber\\
...
\begin{equation}
	K_T = \frac{1}{\rho^*} \left(\frac{\partial P}{\partial \rho^*}\right)^{-1}_T
\end{equation}
and 
\begin{equation}
	K^*_T = \frac{1}{\rho^*} \left(\frac{\partial P^*}{\partial \rho^*}\right)^{-1}_T.
\end{equation}

Figure~\ref{fig2a} illustrates the dependency of the compressibility $K^*_T$ on the density $\rho^*$ for various values of temperature above the critical one. The dependency of $K^*_T$ on the effective chemical potential $M$ is displayed in Fig~\ref{fig2b}.
...
\begin{figure}[htbp]
	...
	%\vskip-3mm
	\caption{The reduced isothermal compressibility $K^*_T$ as a function of the density $\rho^*$ at different values of reduced temperature $\tau > 0$ ($T > T_c$). 
	}
	\label{fig2a}
\end{figure}
...
\begin{figure}[h!]
	...
	\vskip-3mm\caption{The reduced thermal pressure coefficient $\beta^*_V$ as a function of the density $\rho^*$ at different values of relative temperature $\tau > 0$ ($T > T_c$). 
	}\label{fig3a}
\end{figure}
\begin{figure}[h!]
...
Figure~\ref{fig3a} shows the dependency of the pressure coefficient $\beta^*_V$ on the density $\rho^*$ for various values of temperature above the critical one. The dependency of $\beta^*_V$ on the effective chemical potential $M$ is displayed in Fig~\ref{fig3b}.
...
\begin{eqnarray}
	\beta^*_V & = & E_\eta (\rho^*,T) + \bar{M} \rho_{n} + \frac{\tilde D(0)}{2} \rho_{n}^2 - \frac{a_4}{24} \rho_{n}^4
	\nonumber\\
