...
Figure~\ref{fig1} shows the isotherms for the pressure $P^*$ versus density $\bar{n}$ (left-hand side) and versus the effective chemical potential $M$ (right-hand side).
\begin{figure}[h!]
	\centering 
...
\begin{equation}
	...
	\left(\frac{\partial \bar{n}}{\partial \mu}\right)^{-1}_{T}
\end{equation}
which\footnote{Compare this equation for $\beta_V$ with Eq.(17) from~\cite{StrokerMeier2021}} is easily calculated based on Eqs.~\eqref{eq:eosMT} and~\eqref{eq:density}.
